...
ID — every request is a single self-contained `tools/call`.
- Never put the token in a command-line argument (visible to any other \
local user via `ps`/`/proc`, and to your own shell history). Read it from \
an environment variable and hand it to curl via `-K`/`--config`, which \
takes it over a file instead — written to a private `mktemp` file, never a \
fixed path, and removed as soon as the request is done.

```bash
BASE="__BASE__"
# Set this yourself before running — a $RENDERDESK_TOKEN already exported
# in your shell environment, a secrets manager, a prompt, whatever you'd
# use for any other credential. Just never as a literal on the command line.
TOKEN="$RENDERDESK_TOKEN"
...
jq -n --rawfile content "__CONTENT_PATH__" \\
  '{jsonrpc:"2.0",id:1,method:"tools/call",params:{name:"publish_artifact",
