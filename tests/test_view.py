...
async def test_code_artifact_is_syntax_highlighted(client):
    ...
    assert "content-security-policy" not in raw_resp.headers
...
async def test_markdown_without_mermaid_keeps_strict_csp(client):
    connection_id = await make_connection()
...
async def test_markdown_code_fence_is_syntax_highlighted(client):
    ...
    # Server-side only — no client JS, no CSP loosening for code alone.
    assert "script-src" not in resp.headers["content-security-policy"]
